...
여기서 첫 소득이 나왔습니다. **안다고 생각했던 알고리즘도 막상 계약으로 적으려면 손이
멈춥니다.** 아는 줄 알았던 것의 대부분이 문장으로 옮길 만큼 정확한 앎이 아니었던 것입니다.
이해하기 쉬운 기초적인 예시를 하나 들면, 교과서적인 설명은 "최단 경로를 구한다"에서
끝나는데, 최단 경로가 여럿이면 어느 것을 돌려줄지는 아무도 말해 주지 않습니다. 구현마다
다르고, 그래서 언어마다 다릅니다.

> 직접 고민하지 않고 답만 암기했다면 코드는 빨리 칠 수 있을지 몰라도, 이렇게 정해지지
> 않은 자리를 정하지는 못합니다. 그 자리를 정하는 일이 설계이고, 암기는 거기까지
> 데려다주지 않습니다.
