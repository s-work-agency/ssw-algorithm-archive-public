...
한계가 드러나는지**를 확인한 자리에서 멈췄습니다. 사이트에 무엇이 담겼는지부터 보고 싶다면
[8절](#8-이-사이트에-이것을-담았습니다)로 건너뛰어도 됩니다.

<!-- site-only-skip-start -->

> **공개 사이트**는 https://s-work-agency.github.io/ssw-algorithm-archive-public/ 에서
> 볼 수 있습니다. 이 저장소가 그대로 서빙됩니다.
...
[7절](#7-우선은-5000-구현에서-멈춥니다)에 있습니다.
...
이 글도 같은 방식으로 썼습니다. 무엇을 어떤 순서로 말할지, 어떤 어조와 어휘를 쓸지는 사람이
정해 가이드했고, 문장을 실제로 짜는 일은 AI가 했습니다. 그 안에서는 자율성을 줬습니다.
표현을 고르고 문단을 배치하는 일까지 맡기고, 사람은 나온 글을 읽고 방향을 다시 잡았습니다.

참조하는 자료라는 전제는 아카이브의 모양도 바로 결정했습니다. **한 알고리즘은 한 파일에
담고, 그 파일만 건네받아도 다른 파일 없이 동작해야** 합니다. 요청한 알고리즘 하나 대신 옆
알고리즘 열 개가 딸려 오는 응답은 쓸모가 없기 때문입니다. 그리고 아카이브에 없는 것을
물어보면 **없다고 말해야** 합니다. 없는 것을 그럴듯하게 채워 주는 도구는 모델의 실수를
줄이는 게 아니라 늘립니다.
...
처음은 C# · Java · JavaScript 세 언어였습니다. 셋을 고른 데는 이유가 둘 있습니다. 하나는
**가장 많이 다뤄 본 언어들**이라는 점입니다. 처음 밟는 절차를 손에 익지 않은 언어로 시작하면,
결과가 어긋났을 때 계약이 모호한 것인지 그 언어를 몰라서인지 구분되지 않습니다. 다른 하나는
**정적 강타입 · JVM · 동적**이라는 축이 서로 충분히 달라서, 계약에 빈틈이 있으면 세 언어
사이에서 먼저 드러난다는 점입니다.
...
flowchart LR
    ...
    M["<b>LLM 모델</b><br/>web · WPF 등에서 MCP 도구로 조회"]
...
스레드 없이 결정적으로 재현할 수 있는가**가 기준이고, 통과하지 못하는 후보는 넣지 않습니다.
대신 시간과 난수를 전부 호출자가 주입하게 만들어서, 캐시 만료나 재시도 지연처럼 시간에
기대는 알고리즘까지 결정적으로 검증되게 했습니다. 파이프라인의 상세는
...
**안다고 생각했던 알고리즘도 막상 계약으로 적으려면 손이 멈춥니다.** 아는 줄 알았던 것의
대부분이 문장으로 옮길 만큼 정확한 앎이 아니었던 것입니다. 이해하기 쉬운 기초적인 예시를
하나 들면, 교과서적인 설명은 "최단 경로를 구한다"에서 끝나는데, 최단 경로가 여럿이면 어느
것을 돌려줄지는 적혀 있지 않습니다. 미리 고민하지 않으면 그 자리는 결국 버그로 한 번
마주치고 나서야 고치게 됩니다.

알고리즘 풀이는 보통 주어진 문제를 풀고 정답지를 맞춰 보는 형태로 연습합니다. 하지만
실무에서 가장 중요한 것은 그 문제 자체를 출제하는 능력이라고 생각합니다. 어떤 상황에서
어떤 알고리즘을 쓸지 정해진 상태에서 푸는 풀이는 코더로서의 능력을 올릴 수는 있어도,
프로그래머라는 관점에서는 반쪽짜리 연습입니다. **계약을 적는 일이 바로 그 출제에
해당합니다.**

이 계약의 중요성을 이해하지 못하면, 아무리 암기를 잘하고 코드를 빨리 쳐도 같은 실수를
반복하게 됩니다. AI에게 구현을 맡길 때 더 싸고 빠른 소형 모델을 두고 추론에 시간과
토큰을 쓰는 큰 모델을 고르는 이유도 같습니다. 비싼 쪽은 코드를 치는 속도가 아니라,
정해지지 않은 자리를 미리 고민하는 일입니다.

---

## 5. 그다음 궁금해진 것은 규모였습니다

계기는 솔직히 말하면 **예고된 주간 사용 한도 초기화**였습니다. 어차피 한도가 초기화되는
김에 한 번 스케일을 키워 볼까 하는 생각으로 본격적으로 붙었고, 그러면서 질문도 하나
늘었습니다. **같은 일을 500종 × 10개 언어 = 5,000개 구현으로 규모를 키우면 속도와 토큰이
얼마나 들까.** 앞의 둘과 달리 이건 해 보기 전에는 감이 전혀 없는 질문이라, 실제로 키워 보는 것
말고는 답을 얻을 방법이 없었습니다.

그래서 **단일 저장소 하나에 전부 넣어 두던 구조를 언어별 저장소로 분할**하고, 알고리즘
100종 남짓을 500종으로, 3개 언어를 10개 언어로 한꺼번에 올렸습니다.

본편은 **약 3~4일**이었고 거의 **풀타임**으로 돌렸습니다. 켜 놓고 잠을 자기도 했습니다. 규모를
키운 이번 작업에서는 사람 손이 간 곳이 거의 없습니다. 메모리가 밀려 I/O가 너무 느려질 때 **한 번씩
재부팅**해 준 게 전부입니다.
...
## 6. 한계가 드러난 곳은 기능이 아니라 통일성이었습니다

스케일을 키워 보니 비용이 예상과 다른 자리에 붙었습니다. **비싼 쪽은 기능이 아니었습니다.**
알고리즘이 무엇을 하는지를 코드로 옮기는 일은 대체로 금방 끝납니다. 시간도 토큰도 크게 든
쪽은 **열 개 언어의 구현을 서로 어긋나지 않게 맞추는 일**이었습니다.

그 비용의 뿌리는 4절에서 적은 계약이었습니다. 계약이 정확하면 열 개 언어가 같은 자리에
서고, 계약이 모호하면 그 모호함이 언어 수만큼 벌어집니다. 규모는 계약의 중요성을 가장
비싼 방식으로 다시 가르쳐 줬습니다.
...
전체를 들고 가야 했고**, 그건 2절에서 정한 방향과 정면으로 어긋납니다. 그래서 열 개 언어
...
알고리즘은 세 언어로 먼저 세우고, 처음 보는 계약 형태를 만나면 한 종을 끝까지 검증한 뒤에
...
이미 키우고 난 뒤에 알았고, 6절의 절차들은 대부분 그 비용을 치르고 나서 사후에 굳힌
것입니다.

그래서 여기서 한 번 끊습니다. 얻으려던 건 개수가 아니라 **어디서 한계가 드러나는지**였고,
그 답은 이미 나왔습니다. 검증된 자료를 쥐여 주면 소형 모델이 어디까지 활용하는지, 규모를
키우면 어디서 먼저 문제가 생기는지, 계약을 적는다는 게 실제로 무엇을 요구하는지.

한계를 보려던 대상에는 **작업을 돌리는 쪽도 처음부터 들어 있었습니다.** 아주 큰 규모로
키우면 자원이 얼마나 들어가고 어디서 무엇이 막히는지, 그것도 확인하려던 것 중 하나였습니다.
2절에서 말한 "AI 도구를 여럿"의 구체는 **Claude · GPT · Grok** 셋이고, 셋을 함께 쓰면서
**서브에이전트를 동시에 최대 50개까지** 띄워 봤습니다. 그 지점에서는 **CPU · 메모리 · I/O가
거의 풀로드**로 걸렸습니다. 5절에서 적은 재부팅은 그 상태의 다른 단면입니다.

**5,000은 그 답을 얻으려고 치른 값이지 그 자체가 결론이 아닙니다.** 규모를 더 키우는 일은 같은
답을 더 비싸게 다시 사는 쪽에 가깝습니다.
...
서버는 없습니다. 3절에 그린 파이프라인 가운데 **사람이 웹으로 보는 부분만 정적 파일로 만들어
그대로 올린 것**이라, 조회 API도 서버도 없이 배포 루트에 놓인 **정적 파일 한 벌**이
전부입니다. 상세 · 소스 본문 · 공식 벡터까지 전부 정적 파일로 함께 실려 있고, 화면은 파일을
받을 때마다 **인덱스가 발표한 sha256과 byteLength에 받은 바이트가 정확히 일치하는지**
확인합니다. 어긋나면 코드 표시도 복사도
실행도 하지 않습니다. 헤더는 서버가 말하는 것이고 해시는 바이트가 말하는 것이라, 무결성
판정으로는 이쪽이 더 강합니다.

화면 안내와 읽는 순서는 [샘플 안내](docs/sample-guide.md)에 있습니다.

> 이 사이트는 **일회성 스냅샷**입니다. 본체가 갱신돼도 따라가지 않고, 이 저장소에는 공개
> 스냅샷과 이 문서들만 담습니다.

---

## 9. 브라우저에서 그 자리에서 돌려 봅니다
...
flowchart LR
    ...
    W["Web Worker<br/>별도 스레드 격리 · 2초 제한"]
    J{"기대값과 같은가"}
    P["통과"]
    F["실패 · 시간 초과 · 실행 오류"]
...
- 실행은 **브라우저에 내장된 별도 스레드(Web Worker 표준 API) 안에서만** 일어납니다. 워커
  스크립트는 파일로 두지 않고 받아 둔 소스 바이트로 그 자리에서 만들어 띄우므로, 실행하려고
  밖으로 나가는 요청이 없습니다. 화면을 그리는 스레드와 분리돼 있어 DOM에 손댈 수 없고,
  무한 루프가 돌아도 화면이 멈추지 않습니다. **2초를 넘기면 밖에서 워커를 종료**합니다.
  협조를 구하는 신호가 아니라 강제 종료라 `while (true)`도 실제로 끊깁니다.
...
이 기능은 [2절](#2-코드를-새로-짜게-하지-않고-꺼내-쓰게-했습니다)의 전제와 곧장 이어집니다. "검증된 구현을 참조해 꺼내
...
값을 하는 자리이기도 합니다. 받은 바이트를 그대로 워커 스크립트로 삼는 것만으로 확인이
끝납니다.

> 덧붙이면 이 기능은 원래 계획에 없었습니다. 작업 중에 AI의 제안으로 올라왔고, 사람이 한 일은 그
> 제안을 받기로 정한 것뿐입니다. 2절에서 말한 가이드도 대개 이런 모양이라, 무엇을 만들지 먼저
> 지시하기보다 올라온 것 중에서 무엇을 남길지 고르는 쪽이었습니다.
...
| 문서 | 역할 |
|---|---|
| [알고리즘이라는 것](docs/algorithm-thoughts.md) | 알고리즘을 무엇이라고 생각해 왔는지, 그 관점이 이 아카이브의 모양으로 어떻게 이어졌는지 |
| [실험 노트](docs/experiment-notes.md) | 이 이야기가 남긴 실무: 조회 도구의 요건, 규모를 키울 때의 절차와 관례, 계약을 적으며 배운 것 |
