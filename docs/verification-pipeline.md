...
| 항목 | 내용 |
|---|---|
| 분류 | 분류(category) · 계열(family). 둘 다 닫힌 목록이 아닌 확장 지점입니다 |
...
| 잡 | 하는 일 |
|---|---|
| 수직 잡 | 두 언어로 계약 하나를 먼저 확인해 매트릭스 전체를 돌리기 전에 조기 실패 |
...
### 같은 벡터를 브라우저가 다시 돌립니다

JavaScript 소스 카드에는 실행기가 붙어 있습니다. 화면이 위 검사를 통과해 받아 둔 **그 소스
바이트**를 브라우저에 내장된 별도 스레드(Web Worker 표준 API)에서 돌리고, 입력은 함께 실린
**공식 벡터의 케이스**를 씁니다. 결과는 그 케이스가 선언한 기대값 또는 기대 오류 코드와
대조합니다.

- 실행하려고 소스를 따로 받아오지 않습니다. **표시되는 코드와 도는 코드가 같은 바이트**여야
  "돌려 봤다"가 무언가를 증명합니다.
- 워커 스크립트도 그 바이트로 그 자리에서 만들어 띄웁니다. 실행하려고 밖으로 나가는 요청이
  없습니다.
- 화면을 그리는 스레드와 분리돼 있어 DOM에 손댈 수 없고, **2초를 넘기면 밖에서 워커를
  종료**합니다. 강제 종료라 무한 루프도 실제로 끊깁니다.
