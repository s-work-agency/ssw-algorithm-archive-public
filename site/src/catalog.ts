...
export type ImplementationStatus = "R" | "I" | "O" | "E";
export type VerificationStatus = "not-run" | "passing" | "failing";
export type CatalogSchemaVersion = "1.0" | "1.1" | "1.2" | "2.0";
export type CatalogLanguageName =
  | "java"
  | "javascript"
  | "csharp"
  | "typescript"
...
export type FeedbackStatus =
  ...
  | "rejected"
  | "unavailable";
export type CatalogAlgorithmKind =
  | "algorithm"
  | "data-structure"
  | "codec"
  | "protocol"
...
export interface CatalogRelationships {
  ...
  readonly specializationOf?: readonly string[];
  readonly sameProblemDifferentMethod?: readonly string[];
  readonly composes?: readonly string[];
}

export interface CatalogSource {
  readonly id: string;
  readonly language: CatalogLanguageName;
  readonly path: string;
  /** 1.1만 카탈로그 안에 원문을 싣는다. 1.2에서는 반드시 없어야 한다. */
  readonly content?: string;
  /** 1.2 원격 정본의 UTF-8 바이트 SHA-256. 소문자 16진수 64자리다. */
  readonly sha256?: string;
  /** 1.2 원격 정본의 원본 UTF-8 바이트 길이. */
...
export interface CatalogLanguage {
  ...
  /** schema 2.0 reserved 구현이 앞으로 사용할 정규화된 source 경로다. */
  readonly plannedSourcePath?: string;
  readonly plannedBasicTestPath?: string;
  /** schema 2.0 operational 구현의 primary+companion source 묶음이다. */
  readonly sourceGroup?: CatalogFileGroup;
  /**
   * schema 2.0 operational 구현의 기본 테스트 원문 manifest다. null은 구현은
   * operational이지만 기본 테스트 파일은 아직 작성 예정이라는 명시적 상태다.
   */
  readonly basicTest?: CatalogFileGroup | null;
  /** schema 2.0은 공식 vector와 언어-local 기본 테스트 검증을 분리한다. */
  readonly vectorVerification?: CatalogVerificationSummary;
  readonly basicTestVerification?: CatalogVerificationSummary;
...
export interface CatalogFileGroup {
  ...
  /** raw path-keyed map을 ASCII path 순서의 배열로 정규화한 결과다. */
  readonly files: readonly CatalogSource[];
}
...
export interface CatalogDocumentation {
...
/**
 * 2.0 contract는 catalog-core가 공개 metadata 객체로 그대로 보존하는 확장 지점이다.
 * 현재 UI가 아는 input/output/errors/mutation만 타입으로 고정하면 producer-valid 확장
 * 키나 빈 객체를 거절하게 되므로, 모델도 의도적으로 opaque object로 유지한다.
 */
export type CatalogContract = Readonly<Record<string, unknown>>;

export interface CatalogContractPresentation {
  readonly input?: Readonly<Record<string, unknown>>;
  readonly output?: Readonly<Record<string, unknown>>;
  readonly errors: readonly unknown[];
  readonly mutation?: string;
  /** 알려진 UI 형태로 안전하게 해석하지 못한 값과 공개 확장 키다. */
...
export interface CatalogTestVector {
  readonly id: string;
  readonly cases: readonly unknown[];
}

export interface CatalogVectorManifest {
  readonly id: string;
...
export interface CatalogAlgorithm {
  readonly id: string;
  readonly kind?: CatalogAlgorithmKind;
  readonly relationships?: CatalogRelationships;
  readonly alternatives?: readonly string[];
  readonly name: string;
  readonly summary: string;
  readonly documentation?: CatalogDocumentation;
  ...
  /** 1.0/1.1 embedded 벡터. 1.2/2.0에서는 빈 배열로 정규화한다. */
  readonly testVectors: readonly CatalogTestVector[];
  /** 1.2 루트 또는 2.0 알고리즘 내부 vector manifest를 정규화해 참조한다. */
  readonly vectorIds?: readonly string[];
  readonly coverage: {
    readonly vectorCount: number;
    readonly caseCount: number;
    readonly verifiedLanguages: number;
    readonly missingBasicTests?: number;
    readonly releaseReadyLanguages?: number;
  };
  readonly feedback: CatalogFeedback;
}

export interface CatalogDocument {
  readonly schemaVersion: CatalogSchemaVersion;
  readonly sources?: readonly CatalogSource[];
  readonly vectors?: readonly CatalogVectorManifest[];
  readonly algorithms: readonly CatalogAlgorithm[];
  readonly coverage: {
    readonly algorithmCount: number;
    readonly vectorCount: number;
    readonly caseCount: number;
    readonly languageImplementations: Readonly<
      Record<ImplementationStatus, number>
    >;
    readonly verifiedLanguages: number;
    readonly implementationStates?: Readonly<{
      reserved: number;
      operational: number;
    }>;
    readonly missingBasicTests?: number;
...
/**
 * 상단 상태줄은 벡터 수처럼 알고리즘 수와 사실상 같은 숫자보다 실제 릴리스
 * 준비도를 보여준다. 이 집계는 화면 문구와 분리해 테스트 가능하게 유지한다.
 */
export interface CatalogReadinessSummary {
  readonly algorithmCount: number;
  readonly sourceCount: number;
  readonly caseCount: number;
  readonly implementationCount: number;
  readonly basicTestPassingImplementations: number;
  readonly releaseReadyImplementations: number;
  /** 인덱스 totals가 이 집계를 싣기 전에 발행된 배포에서는 비어 있다. */
...
export interface FeedbackSummary {
  readonly statusCounts: Readonly<Record<FeedbackStatus, number>>;
  readonly evidenceCount: number;
  readonly candidateCount: number;
  readonly approvedCount: number;
  readonly rejectedCount: number;
...
const statuses = ["R", "I", "O", "E"] as const;
const verificationStatuses = ["not-run", "passing", "failing"] as const;
const catalogSchemaVersions = ["1.0", "1.1", "1.2", "2.0"] as const;
const languages = [
  "java",
  "javascript",
  "csharp",
...
const sourceExtensions: Readonly<Record<CatalogLanguageName, readonly string[]>> = {
  java: [".java"],
  javascript: [".js"],
  csharp: [".cs"],
  typescript: [".ts"],
 ...
 */
export function contractPresentation(
  contract: CatalogContract,
): CatalogContractPresentation {
  ...
  const mutation =
    typeof contract.mutation === "string" ? contract.mutation : undefined;
  const opaque = Object.fromEntries(
    Object.entries(contract).filter(([key, value]) => {
      if (key === "input") return !isRecord(value);
      if (key === "output") return !isRecord(value);
      if (key === "errors") return !Array.isArray(value);
      if (key === "mutation") return typeof value !== "string";
      return true;
    }),
  );
  return {
...
function string(value: unknown, field: string): string {
  if (typeof value !== "string" || !value.trim()) {
    throw new TypeError(
      `catalog.json ${field}: 비어 있지 않은 문자열이어야 합니다.`,
    );
  }
  return value.trim();
}

function sourceContent(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`catalog.json ${field}: 문자열이어야 합니다.`);
  }
  if (new TextEncoder().encode(value).byteLength > maxSourceBytes) {
    throw new TypeError(
      `catalog.json ${field}: UTF-8 기준 ${maxSourceBytes}바이트 이하여야 합니다.`,
    );
  }
  if (
    value
    ...
    throw new TypeError(
      `catalog.json ${field}: 각 줄은 ${maxSourceLineCharacters}자 이하여야 합니다.`,
    );
  }
  return value;
}
...
function exactKeys(
  ...
  if (unknown.length) {
    throw new TypeError(
      `catalog.json ${field}: 알 수 없는 키가 있습니다: ${unknown.join(", ")}.`,
    );
  }
  const missing = allowedKeys.filter((key) => !Object.hasOwn(value, key));
  if (missing.length) {
    throw new TypeError(
      `catalog.json ${field}: 필수 키가 없습니다: ${missing.join(", ")}.`,
    );
  }
}
...
function sha256(value: unknown, field: string): string {
  if (typeof value !== "string" || !/^[0-9a-f]{64}$/u.test(value)) {
    throw new TypeError(
      `catalog.json ${field}: 소문자 16진수 SHA-256 64자리여야 합니다.`,
    );
  }
  return value;
}

function languageName(value: unknown, field: string): CatalogLanguageName {
  const parsed = string(value, field);
  if (!languages.includes(parsed as CatalogLanguageName)) {
    throw new TypeError(
      `catalog.json ${field}: 지원하는 구현 언어여야 합니다.`,
    );
  }
  return parsed as CatalogLanguageName;
}

function count(value: unknown, field: string): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0) {
    throw new TypeError(
      `catalog.json ${field}: 0 이상의 정수여야 합니다.`,
    );
  }
  return value;
}
...
function exactText(value: unknown, field: string): string {
  if (
    ...
    throw new TypeError(
      `catalog.json ${field}: 앞뒤 공백 없는 비어 있지 않은 문자열이어야 합니다.`,
    );
  }
  return value;
}
...
function positiveCount(
  ...
  if (maximum !== undefined && parsed > maximum) {
    throw new TypeError(
      `catalog.json ${field}: ${maximum} 이하여야 합니다.`,
    );
  }
  return parsed;
}
...
function asciiSortedKeys(value: RecordValue, field: string): readonly string[] {
  ...
  for (let index = 1; index < keys.length; index += 1) {
    ...
    if (previous === undefined || current === undefined || previous >= current) {
      throw new TypeError(
        `catalog.json ${field}: 키는 ASCII 오름차순이며 중복될 수 없습니다.`,
      );
    }
  }
  return keys;
...
function algorithmIdentifier20(value: unknown, field: string): string {
  if (
    typeof value !== "string" ||
    !/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/u.test(value)
  ) {
    throw new TypeError(
      `catalog.json ${field}: 영문 소문자로 시작하는 kebab-case 식별자여야 합니다.`,
    );
  }
  return value;
}

function kebabIdentifier(value: unknown, field: string): string {
  if (
    typeof value !== "string" ||
    !/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(value)
  ) {
    throw new TypeError(
      `catalog.json ${field}: 소문자 kebab-case 식별자여야 합니다.`,
    );
  }
  return value;
}
...
function exactTextArray(value: unknown, field: string): readonly string[] {
  if (!Array.isArray(value)) {
    throw new TypeError(`catalog.json ${field}: 배열이어야 합니다.`);
  }
...
): CatalogDocumentation | undefined {
  if (value === undefined) return undefined;
  if (schemaVersion === "1.0") {
    throw new TypeError(
      `catalog.json ${field}: documentation은 1.1 이상 계약에서만 사용할 수 있습니다.`,
    );
  }
...
function parseAlgorithmKind(
  value: unknown,
  field: string,
  schemaVersion: CatalogSchemaVersion,
): CatalogAlgorithmKind | undefined {
  if (value === undefined) return undefined;
  if (schemaVersion !== "1.2" && schemaVersion !== "2.0") {
    throw new TypeError(
      `catalog.json ${field}: kind는 1.2/2.0 계약에서만 사용할 수 있습니다.`,
    );
  }
  const parsed = string(value, field);
  if (!algorithmKinds.includes(parsed as CatalogAlgorithmKind)) {
    throw new TypeError(`catalog.json ${field}: 허용되지 않는 algorithm kind입니다.`);
  }
  return parsed as CatalogAlgorithmKind;
}
...
function parseRelationships(
  ...
  field: string,
  schemaVersion: CatalogSchemaVersion,
): CatalogRelationships | undefined {
  if (value === undefined) return undefined;
  if (schemaVersion !== "1.2" && schemaVersion !== "2.0") {
    throw new TypeError(
      `catalog.json ${field}: relationships는 1.2/2.0 계약에서만 사용할 수 있습니다.`,
    );
  }
  ...
  if (!Object.keys(relationships).length) {
    throw new TypeError(
      `catalog.json ${field}: 관계 키를 하나 이상 선언해야 합니다.`,
    );
  }
  ...
  for (const relationship of relationshipFields) {
    ...
    const ids = uniqueStrings(
      raw.map((target, index) =>
        kebabIdentifier(target, `${field}.${relationship}[${index}]`),
      ),
      `${field}.${relationship}`,
    );
    if (!ids.length) {
      throw new TypeError(
        `catalog.json ${field}.${relationship}: 하나 이상의 algorithmId가 필요합니다.`,
      );
    }
    parsed[relationship] = ids;
  }
  return parsed as CatalogRelationships;
}
...
function parseSource(
  ...
  if (schemaVersion === "1.2") {
    exactKeys(
      source,
      ["id", "language", "path", "sha256", "byteLength"],
      field,
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      `catalog.json ${field}.path: 선언 언어의 정규화된 공개 소스 경로여야 합니다.`,
    );
  }
  const id = string(source.id, `${field}.id`);
  if (id !== `source:${path}`) {
    throw new TypeError(
      `catalog.json ${field}.id: source:<path> 형식과 path가 일치해야 합니다.`,
    );
  }
  const common = {
    id,
  ...
  if (schemaVersion === "1.2") {
    ...
    if (parsedByteLength > maxSourceBytes) {
      throw new TypeError(
        `catalog.json ${field}.byteLength: ${maxSourceBytes}바이트 이하여야 합니다.`,
      );
    }
    return {
      ...common,
...
function parseVectorManifest(
  value: unknown,
  index: number,
): CatalogVectorManifest {
  const field = `vectors[${index}]`;
  const vector = object(value, field);
  exactKeys(
    vector,
    ["id", "algorithmId", "path", "sha256", "byteLength", "caseCount"],
    field,
  );
  ...
  const algorithmId = kebabIdentifier(
    vector.algorithmId,
    `${field}.algorithmId`,
  );
  if (path !== `test-vectors/valid/${id}.json`) {
    throw new TypeError(
      `catalog.json ${field}.path: 'test-vectors/valid/${id}.json'과 같아야 합니다.`,
    );
  }
...
function parseLanguage(
  ...
  if (!statuses.includes(implementationStatus as ImplementationStatus)) {
    throw new TypeError(
      `catalog.json ${field}.implementationStatus: 허용되지 않는 상태입니다.`,
    );
  }
  if (!verificationStatuses.includes(verification as VerificationStatus)) {
    throw new TypeError(
      `catalog.json ${field}.verification: 허용되지 않는 검증 상태입니다.`,
    );
  }
  const summary = object(language.testSummary, `${field}.testSummary`);
  const sourceId =
    language.sourceId === undefined
      ? undefined
      : string(language.sourceId, `${field}.sourceId`);
  if (schemaVersion === "1.0" && sourceId !== undefined) {
    throw new TypeError(
      `catalog.json ${field}.sourceId: sourceId는 1.1/1.2 계약에서만 사용할 수 있습니다.`,
    );
  }
  return {
    language: languageName(language.language, `${field}.language`),
    implementationStatus: implementationStatus as ImplementationStatus,
...
function parseContract(value: unknown, field: string): CatalogContract {
...
function parseFeedback(
  ...
  if (!feedbackStatuses.includes(status as FeedbackStatus)) {
    throw new TypeError(
      `catalog.json ${field}.status: 허용되지 않는 피드백 상태입니다.`,
    );
  }
  ...
  const suppliedExtendedCount = feedbackCountFields.some(
    (countField) => feedback[countField] !== undefined,
  );
  ...
  if (status === "unavailable") {
    if (suppliedExtendedCount || evidenceCount !== 0) {
      throw new TypeError(
        `catalog.json ${field}: unavailable 피드백은 evidenceCount=0인 레거시 형태여야 합니다.`,
      );
    }
    return {
      status: "unavailable",
  ...
  if (schemaVersion === "1.0") {
    if (suppliedExtendedCount || status !== "unavailable") {
      throw new TypeError(
        `catalog.json ${field}: 1.0 계약은 unavailable 피드백만 허용합니다.`,
      );
    }
  }
  if (!suppliedExtendedCount) {
    throw new TypeError(
      `catalog.json ${field}: 1.1 피드백 집계 필드를 모두 선언해야 합니다.`,
    );
  }

  const parsed: CatalogFeedback = {
    status: status as FeedbackStatus,
    evidenceCount,
    candidateCount: count(
      feedback.candidateCount,
      `${field}.candidateCount`,
    ),
    approvedCount: count(
      feedback.approvedCount,
      `${field}.approvedCount`,
    ),
    rejectedCount: count(
  ...
  if (parsed.approvedCount + parsed.rejectedCount > parsed.candidateCount) {
    throw new TypeError(
      `catalog.json ${field}: 승인·거절 수는 후보 수를 넘을 수 없습니다.`,
    );
  }
  const allCountsAreZero =
  ...
  if (parsed.status === "none" && !allCountsAreZero) {
    throw new TypeError(
      `catalog.json ${field}: none 상태의 집계는 모두 0이어야 합니다.`,
    );
  }
  if (parsed.status === "pending" && parsed.pendingCount === 0) {
    throw new TypeError(
      `catalog.json ${field}.pendingCount: pending 상태에서는 1 이상이어야 합니다.`,
    );
  }
  if (
    ...
    throw new TypeError(
      `catalog.json ${field}: approved 상태에는 승인 1건 이상과 pending 0건이 필요합니다.`,
    );
  }
  if (
    ...
    throw new TypeError(
      `catalog.json ${field}: rejected 상태에는 거절 1건 이상만 있어야 합니다.`,
    );
  }
  return parsed;
}
...
function parseAlgorithm(
  ...
  if (!Array.isArray(item.languages)) {
    throw new TypeError(
      `catalog.json ${field}: 목록 필드가 올바르지 않습니다.`,
    );
  }
  if (schemaVersion === "1.2") {
    if (!Array.isArray(item.vectorIds) || item.testVectors !== undefined) {
      throw new TypeError(
        `catalog.json ${field}: 1.2는 testVectors 대신 vectorIds 배열을 사용해야 합니다.`,
      );
    }
  } else if (!Array.isArray(item.testVectors) || item.vectorIds !== undefined) {
    throw new TypeError(
      `catalog.json ${field}: 1.0/1.1은 embedded testVectors 배열을 사용해야 합니다.`,
    );
  }
  ...
  if (relationships !== undefined && kind === undefined) {
    throw new TypeError(
      `catalog.json ${field}.relationships: kind가 있는 metadata 1.1 알고리즘에만 사용할 수 있습니다.`,
    );
  }
  ...
  const languages = item.languages.map((language, languageIndex) =>
    parseLanguage(
      language,
      `${field}.languages[${languageIndex}]`,
      schemaVersion,
    ),
  );
  uniqueStrings(
  ...
  const testVectors = embeddedVectors.map((vector, vectorIndex) => {
    ...
    if (!Array.isArray(entry.cases)) {
      throw new TypeError(
        `catalog.json ${vectorField}.cases: 배열이어야 합니다.`,
      );
    }
    return {
  ...
  uniqueStrings(
    testVectors.map((vector) => vector.id),
    `${field}.testVectors[].id`,
  );
  const vectorIds =
    schemaVersion === "1.2"
      ? (item.vectorIds as readonly unknown[]).map((vectorId, vectorIndex) =>
          kebabIdentifier(vectorId, `${field}.vectorIds[${vectorIndex}]`),
        )
      : undefined;
  if (vectorIds) {
    for (let index = 1; index < vectorIds.length; index += 1) {
      ...
      if (
        ...
        throw new TypeError(
          `catalog.json ${field}.vectorIds: 중복 없이 오름차순이어야 합니다.`,
        );
      }
    }
  }
  ...
  return {
    ...
    coverage: {
      ...
      verifiedLanguages: count(
        coverage.verifiedLanguages,
        `${field}.coverage.verifiedLanguages`,
      ),
    },
...
interface Catalog20ParseState {
  readonly artifactIds: Set<string>;
  readonly artifactPaths: Set<string>;
  readonly vectorIds: Set<string>;
  readonly vectorPaths: Set<string>;
  readonly sources: CatalogSource[];
  readonly vectors: CatalogVectorManifest[];
}

function manifestPath20(
  value: unknown,
  language: CatalogLanguageName,
  root: "main" | "test",
  field: string,
): string {
  ...
  if (
    ...
    !sourceExtensions[language].some((extension) =>
      parsed.endsWith(extension),
    )
  ) {
    throw new TypeError(
      `catalog.json ${field}: '${prefix}' 아래의 정규화된 POSIX 소스 경로여야 합니다.`,
    );
  }
  return parsed;
}

function parseVerificationSummary20(
  value: unknown,
  field: string,
  ...
  if (!verificationStatuses.includes(status as VerificationStatus)) {
    throw new TypeError(
      `catalog.json ${field}.status: 허용되지 않는 검증 상태입니다.`,
    );
  }
  ...
  if (!validStatus) {
    throw new TypeError(
      `catalog.json ${field}.status: not-run은 전부 0, passing은 passed>0/failed=0, failing은 failed>0이어야 합니다.`,
    );
  }
  return {
 ...
 */
function parseFileGroup20(
  value: unknown,
  language: CatalogLanguageName,
  root: "main" | "test",
  field: string,
  state: Catalog20ParseState,
): CatalogFileGroup {
  ...
  if (!paths.length) {
    throw new TypeError(
      `catalog.json ${field}.files: 하나 이상의 파일이 필요합니다.`,
    );
  }
  if (!Object.hasOwn(fileMap, primaryFile)) {
    throw new TypeError(
      `catalog.json ${field}.primaryFile: files map key에 존재해야 합니다.`,
    );
  }

  const files = paths.map((pathKey): CatalogSource => {
    ...
    if (declaredPath !== path) {
      throw new TypeError(
        `catalog.json ${fileField}.path: files map key와 같아야 합니다.`,
      );
    }
    const id = exactText(manifest.id, `${fileField}.id`);
    if (id !== `source:${path}`) {
      throw new TypeError(
        `catalog.json ${fileField}.id: source:<path>와 같아야 합니다.`,
      );
    }
    if (state.artifactIds.has(id) || state.artifactPaths.has(path)) {
      throw new TypeError(
        `catalog.json ${fileField}: artifact ID와 path는 catalog 전체에서 한 구현만 소유해야 합니다.`,
      );
    }
    const parsed: CatalogSource = {
      id,
      language,
      path,
...
function parseImplementation20(
  value: unknown,
  language: CatalogLanguageName,
  field: string,
  state: Catalog20ParseState,
  ...
  if (status === "reserved") {
    ...
    const plannedEntryPoint = exactText(
      implementation.plannedEntryPoint,
      `${field}.plannedEntryPoint`,
    );
    return {
      language,
      implementationStatus: "R",
      entryPoint: plannedEntryPoint,
      plannedSourcePath: manifestPath20(
        implementation.plannedSourcePath,
        language,
        "main",
        `${field}.plannedSourcePath`,
      ),
      plannedBasicTestPath: manifestPath20(
        implementation.plannedBasicTestPath,
        language,
        "test",
        `${field}.plannedBasicTestPath`,
      ),
  ...
  if (status !== "operational") {
    throw new TypeError(
      `catalog.json ${field}.status: operational 또는 reserved여야 합니다.`,
    );
  }
  exactKeys(
    implementation,
    ["status", "entryPoint", "source", "basicTest", "verification"],
    field,
  );
  const sourceGroup = parseFileGroup20(
  ...
  const vectorVerification = parseVerificationSummary20(
    verification.vectors,
    `${field}.verification.vectors`,
  );
  const basicTestVerification = parseVerificationSummary20(
    verification.basicTest,
    `${field}.verification.basicTest`,
  );
  if (
    basicTest === null &&
    ...
    throw new TypeError(
      `catalog.json ${field}.verification.basicTest: basicTest가 null이면 not-run/0이어야 합니다.`,
    );
  }
  const primarySource = sourceGroup.files.find(
    (source) => source.path === sourceGroup.primaryFile,
  );
  if (!primarySource) {
    // 위 primaryFile membership 검사가 있어 정상 경로에서는 도달하지 않는다. 타입
    // 좁히기를 위해서도 fail-closed를 유지한다.
    throw new TypeError(
      `catalog.json ${field}.source.primaryFile: primary manifest를 찾을 수 없습니다.`,
    );
  }
  return {
    language,
...
function parseDocumentation20(
  ...
  exactKeys(
    documentation,
    ["summary", "whenToUse", "avoidWhen", "tradeoffs", "pitfalls"],
    field,
  );
  const list = (key: "whenToUse" | "avoidWhen" | "tradeoffs" | "pitfalls") =>
    uniqueStrings(
      exactTextArray(documentation[key], `${field}.${key}`),
      `${field}.${key}`,
    );
  return {
...
function parseRelationships20(
  ...
  const parsed: Partial<Record<(typeof relationshipFields)[number], readonly string[]>> = {};
  for (const relationship of relationshipFields) {
    if (!Object.hasOwn(relationshipObject, relationship)) continue;
    const rawTargets = relationshipObject[relationship];
    if (!Array.isArray(rawTargets) || rawTargets.length === 0) {
      throw new TypeError(
        `catalog.json ${field}.${relationship}: 하나 이상의 algorithmId 배열이어야 합니다.`,
      );
    }
    parsed[relationship] = uniqueStrings(
      rawTargets.map((target, index) =>
        algorithmIdentifier20(
          target,
...
function parseFeedback20(value: unknown, field: string): CatalogFeedback {
  const feedback = object(value, field);
...
function parseVectors20(
  ...
  return asciiSortedKeys(vectorMap, field).map((rawVectorId) => {
    ...
    exactKeys(
      vector,
      ["path", "sha256", "byteLength", "caseCount"],
      vectorField,
    );
    const path = exactText(vector.path, `${vectorField}.path`);
    if (path !== `test-vectors/valid/${vectorId}.json`) {
      throw new TypeError(
        `catalog.json ${vectorField}.path: vector key와 일치해야 합니다.`,
      );
    }
    if (state.vectorIds.has(vectorId) || state.vectorPaths.has(path)) {
      throw new TypeError(
        `catalog.json ${vectorField}: vector ID와 path는 catalog 전체에서 중복될 수 없습니다.`,
      );
    }
    const parsed: CatalogVectorManifest = {
      id: vectorId,
      algorithmId,
      path,
...
function parseAlgorithm20(
  ...
  if (item.kind !== undefined) exactText(item.kind, `${field}.kind`);
  const kind = parseAlgorithmKind(item.kind, `${field}.kind`, "2.0");
  const relationships = parseRelationships20(
    item.relationships,
    `${field}.relationships`,
  );
  if (relationships !== undefined && kind === undefined) {
    throw new TypeError(
      `catalog.json ${field}.relationships: kind가 있는 알고리즘에만 사용할 수 있습니다.`,
    );
  }
  ...
  const alternatives = uniqueStrings(
    exactTextArray(item.alternatives, `${field}.alternatives`).map(
      (target, index) =>
        algorithmIdentifier20(target, `${field}.alternatives[${index}]`),
    ),
    `${field}.alternatives`,
  );
  if (alternatives.includes(algorithmId)) {
    throw new TypeError(
      `catalog.json ${field}.alternatives: 자기 자신을 참조할 수 없습니다.`,
    );
  }

  const category = exactText(item.category, `${field}.category`);
  if (!(catalogCategories20 as readonly string[]).includes(category)) {
    throw new TypeError(
      `catalog.json ${field}.category: 정본 category enum이어야 합니다.`,
    );
  }
  ...
  const parsedContract: CatalogContract = contract;
  const complexity = object(item.complexity, `${field}.complexity`);
  ...
  const parsedLanguages = languages.map((language) =>
    parseImplementation20(
      implementations[language],
      language,
      `${field}.implementations.${language}`,
      state,
    ),
  );
  const vectorIds = parseVectors20(
    item.vectors,
    algorithmId,
    `${field}.vectors`,
    state,
  );
  const manifestVectors = state.vectors.filter(
    (vector) => vector.algorithmId === algorithmId,
  );
  const verifiedLanguages = parsedLanguages.filter(
    (language) =>
      language.implementationStatus === "O" &&
      language.vectorVerification?.status === "passing",
  ).length;
  const missingBasicTests = parsedLanguages.filter(
    (language) =>
      language.implementationStatus === "O" && language.basicTest === null,
  ).length;
  const releaseReadyLanguages = parsedLanguages.filter(
    (language) =>
      language.implementationStatus === "O" &&
      ...
      language.basicTestVerification.skipped === 0,
  ).length;
  ...
  return {
    ...
    documentation: parseDocumentation20(
      item.documentation,
      `${field}.documentation`,
    ),
    category,
    ...
    preconditions: item.preconditions.map((condition, index) =>
      exactText(condition, `${field}.preconditions[${index}]`),
    ),
    ...
    coverage: {
      ...
      caseCount: manifestVectors.reduce(
        (total, vector) => total + vector.caseCount,
        0,
      ),
      verifiedLanguages,
      missingBasicTests,
...
function parseCatalog20(root: RecordValue): CatalogDocument {
  exactKeys(root, ["schemaVersion", "algorithms"], "$");
  if (root.schemaVersion !== "2.0") {
    throw new TypeError(
      'catalog.json $.schemaVersion: "2.0"이어야 합니다.',
    );
  }
  const algorithmMap = object(root.algorithms, "$.algorithms");
  const algorithmIds = asciiSortedKeys(algorithmMap, "$.algorithms").map(
    (id) => algorithmIdentifier20(id, `$.algorithms.${id}`),
  );
  if (!algorithmIds.length) {
    throw new TypeError(
      "catalog.json $.algorithms: 하나 이상의 알고리즘이 필요합니다.",
    );
  }
  const state: Catalog20ParseState = {
    artifactIds: new Set(),
  ...
  for (const algorithm of algorithms) {
    for (const targetId of algorithm.alternatives ?? []) {
      if (!algorithmIdSet.has(targetId)) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.alternatives: 존재하지 않는 algorithmId '${targetId}'을 참조합니다.`,
        );
      }
    }
    for (const relationship of relationshipFields) {
      for (const targetId of algorithm.relationships?.[relationship] ?? []) {
        if (targetId === algorithm.id) {
          throw new TypeError(
            `catalog.json ${algorithm.id}.relationships.${relationship}: 자기 자신을 참조할 수 없습니다.`,
          );
        }
        if (!algorithmIdSet.has(targetId)) {
          throw new TypeError(
            `catalog.json ${algorithm.id}.relationships.${relationship}: 존재하지 않는 algorithmId '${targetId}'을 참조합니다.`,
          );
        }
      }
    }
  ...
  const languageImplementations: Record<ImplementationStatus, number> = {
    R: 0,
    I: 0,
    O: 0,
    E: 0,
  ...
  const caseCount = state.vectors.reduce(
    (total, vector) => total + vector.caseCount,
    0,
  );
  return {
...
function assertEqualCount(
  ...
  if (actual !== expected) {
    throw new TypeError(
      `catalog.json ${field}: 선언값 ${actual}과 계산값 ${expected}이 일치하지 않습니다.`,
    );
  }
}
...
function validateRelationships(
  algorithms: readonly CatalogAlgorithm[],
): void {
  ...
  for (const algorithm of algorithms) {
    for (const targetId of
      algorithm.relationships?.sameProblemDifferentMethod ?? []) {
      const target = algorithmsById.get(targetId);
      if (
        !target?.relationships?.sameProblemDifferentMethod?.includes(
          algorithm.id,
        )
      ) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.relationships.sameProblemDifferentMethod: '${targetId}'와의 관계는 양방향이어야 합니다.`,
        );
      }
    }
  }
  ...
  for (const relationship of acyclicRelationshipFields) {
    ...
    const visit = (id: string): void => {
      ...
      if (currentState === 1) {
        ...
        throw new TypeError(
          `catalog.json algorithms: ${relationship} 관계는 순환할 수 없습니다: ${cycle.join(" -> ")}.`,
        );
      }
      state.set(id, 1);
      stack.push(id);
      for (const targetId of
        algorithmsById.get(id)?.relationships?.[relationship] ?? []) {
        visit(targetId);
      }
      stack.pop();
      state.set(id, 2);
    };
    for (const id of [...algorithmsById.keys()].sort((left, right) =>
...
function validateCoverage(
  algorithms: readonly CatalogAlgorithm[],
  coverage: CatalogDocument["coverage"],
  vectors: readonly CatalogVectorManifest[] = [],
): void {
  ...
  for (const algorithm of algorithms) {
    const manifestVectors = (algorithm.vectorIds ?? []).map((id) => {
      ...
      if (!vector) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.vectorIds: 존재하지 않는 vector '${id}'를 참조합니다.`,
        );
      }
      return vector;
    });
    ...
    const algorithmCaseCount = algorithm.vectorIds
      ...
      : algorithm.testVectors.reduce(
          (total, vector) => total + vector.cases.length,
          0,
        );
    const algorithmVerifiedLanguages = algorithm.languages.filter(
      (language) => language.verification === "passing",
    ).length;
    assertEqualCount(
      algorithm.coverage.vectorCount,
      algorithmVectorCount,
      `${algorithm.id}.coverage.vectorCount`,
    );
    assertEqualCount(
      algorithm.coverage.caseCount,
      algorithmCaseCount,
      `${algorithm.id}.coverage.caseCount`,
    );
    assertEqualCount(
      algorithm.coverage.verifiedLanguages,
      algorithmVerifiedLanguages,
      `${algorithm.id}.coverage.verifiedLanguages`,
    );
  ...
  for (const status of statuses) {
    assertEqualCount(
      coverage.languageImplementations[status],
      languageImplementations[status],
      `coverage.languageImplementations.${status}`,
    );
  }
}
...
export function parseCatalog(value: unknown): CatalogDocument {
  ...
  if (schemaVersion === "2.0") return parseCatalog20(root);
  const coverageValue = object(root.coverage, "coverage");
  if (
    !catalogSchemaVersions.includes(schemaVersion as CatalogSchemaVersion) ||
    !Array.isArray(root.algorithms)
  ) {
    throw new TypeError(
      "catalog.json은 공개된 1.0, 1.1, 1.2 또는 2.0 계약을 만족해야 합니다.",
    );
  }
  const parsedSchemaVersion = schemaVersion as CatalogSchemaVersion;
  if (parsedSchemaVersion === "1.2") {
    // API base 같은 배포 설정이나 본문 필드가 root에 스며드는 것도 차단한다.
    exactKeys(
      root,
      ["schemaVersion", "sources", "vectors", "algorithms", "coverage"],
      "$",
    );
  }
  if (parsedSchemaVersion === "1.0" && root.sources !== undefined) {
    throw new TypeError(
      "catalog.json sources: sources는 1.1/1.2 계약에서만 사용할 수 있습니다.",
    );
  }
  if (parsedSchemaVersion === "1.2" && root.sources === undefined) {
    throw new TypeError(
      "catalog.json sources: 1.2 계약에서는 source manifest 배열이 필요합니다.",
    );
  }
  if (parsedSchemaVersion === "1.2" && !Array.isArray(root.vectors)) {
    throw new TypeError(
      "catalog.json vectors: 1.2 계약에서는 vector manifest 배열이 필요합니다.",
    );
  }
  if (parsedSchemaVersion !== "1.2" && root.vectors !== undefined) {
    throw new TypeError(
      "catalog.json vectors: vectors는 1.2 계약에서만 사용할 수 있습니다.",
    );
  }
  ...
  const sources =
    root.sources === undefined
      ? undefined
      : root.sources.map((source, index) =>
          parseSource(source, index, parsedSchemaVersion),
        );
  ...
  if (vectors) {
    ...
    for (let index = 1; index < vectors.length; index += 1) {
      ...
      if (
        ...
        throw new TypeError(
          "catalog.json vectors: path 기준 중복 없는 오름차순이어야 합니다.",
        );
      }
    }
  }

  const languageImplementationsValue = object(
    coverageValue.languageImplementations,
    "coverage.languageImplementations",
  );
  const languageImplementations = Object.fromEntries(
    statuses.map((status) => [
      status,
      count(
        languageImplementationsValue[status],
        `coverage.languageImplementations.${status}`,
      ),
    ]),
  ) as Record<ImplementationStatus, number>;
  const algorithms = root.algorithms.map((algorithm, index) =>
    parseAlgorithm(algorithm, index, parsedSchemaVersion),
  );
  uniqueStrings(
  ...
  for (const algorithm of algorithms) {
    for (const alternativeId of algorithm.alternatives ?? []) {
      if (alternativeId === algorithm.id) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.alternatives: 자기 자신을 대안으로 참조할 수 없습니다.`,
        );
      }
      if (!algorithmIds.has(alternativeId)) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.alternatives: 존재하지 않는 대안 algorithmId '${alternativeId}'을 참조합니다.`,
        );
      }
    }
    for (const relationship of relationshipFields) {
      for (const relatedId of algorithm.relationships?.[relationship] ?? []) {
        ...
        if (!algorithmIds.has(relatedId)) {
          throw new TypeError(
            `catalog.json ${algorithm.id}.relationships.${relationship}: 존재하지 않는 algorithmId '${relatedId}'을 참조합니다.`,
          );
        }
      }
    }
    for (const language of algorithm.languages) {
      if (!language.sourceId) continue;
      const source = sourcesById.get(language.sourceId);
      if (!source) {
        throw new TypeError(
          `catalog.json ${algorithm.id}/${language.language}.sourceId: 존재하지 않는 source를 참조합니다.`,
        );
      }
      if (source.language !== language.language) {
        throw new TypeError(
          `catalog.json ${algorithm.id}/${language.language}.sourceId: source 언어와 구현 언어가 일치하지 않습니다.`,
        );
      }
    ...
    for (const vectorId of algorithm.vectorIds ?? []) {
      ...
      if (!vector) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.vectorIds: 존재하지 않는 vector '${vectorId}'를 참조합니다.`,
        );
      }
      if (vector.algorithmId !== algorithm.id) {
        throw new TypeError(
          `catalog.json ${algorithm.id}.vectorIds: vector algorithmId가 참조 알고리즘과 일치하지 않습니다.`,
        );
      }
  ...
  if (parsedSchemaVersion === "1.2" && sources) {
    ...
    if (unreferenced.length) {
      throw new TypeError(
        `catalog.json sources: 어떤 언어에서도 참조하지 않는 source가 있습니다: ${unreferenced.join(", ")}.`,
      );
    }
  }
  if (vectors && referencedVectorIds.size !== vectors.length) {
    ...
    throw new TypeError(
      `catalog.json vectors: 알고리즘이 참조하지 않는 vector가 있습니다: ${unreferenced.join(", ")}.`,
    );
  }

  const coverage: CatalogDocument["coverage"] = {
    algorithmCount: count(
      coverageValue.algorithmCount,
      "coverage.algorithmCount",
    ),
    ...
    languageImplementations,
    verifiedLanguages: count(
      coverageValue.verifiedLanguages,
      "coverage.verifiedLanguages",
    ),
  };
  validateRelationships(algorithms);
...
/* ────────────────────────────────────────────────────────────────────────────
 * schema 2.1 — 목록 인덱스 + 종별 상세 + 검색 보조
 *
 * 2.0 단일본은 목록 첫 화면에도 300종의 구현·벡터·계약을 전부 내려받게 만든다. 2.1은
 * 같은 정본을 세 층으로 나눠 게시하고, 소비자도 그 순서대로 읽는다.
 *   catalog-index.json      첫 요청. 목록·커버리지·피드백·로드맵·상단 지표가 여기서 나온다
 *   catalog-search.json     검색 시점(또는 유휴 시점) 지연 로드. documentation 네 목록
 *   algorithms/<id>.json    상세 진입 시점 지연 로드. 그 한 종의 나머지 전부
 *
 * 상세·검색 보조는 인덱스가 sha256·byteLength manifest로 가리키는 경로형 정적 파일이다.
 * 원격 소스·벡터와 달리 서버 헤더 계약이 없으므로, 무결성은 받은 바이트의 해시와 길이
 * 대조만으로 확정한다(remote-assets.ts의 loadVerifiedStaticJson).
 *
 * 아래 진단 메시지의 `catalog.json` 접두는 물리 파일이 아니라 catalog 데이터 계약을
 * 가리킨다. 실제로 어느 파일을 읽다 실패했는지는 로더가 콘솔에 URL로 남긴다.
 * ──────────────────────────────────────────────────────────────────────────── */

/** 경로형 정적 자산의 최소 무결성 manifest. 상세·검색 보조가 같은 모양을 쓴다. */
...
/** 인덱스가 아는 언어 정보는 이름과 상태뿐이다. 진입점·소스는 상세에만 있다. */
export interface CatalogLanguageStatus {
  readonly language: CatalogLanguageName;
  readonly implementationStatus: ImplementationStatus;
}

export interface CatalogIndexTotals {
  readonly algorithmCount: number;
  readonly sourceCount: number;
  readonly vectorCount: number;
  readonly caseCount: number;
  readonly implementationCount: number;
  readonly operationalImplementations: number;
  readonly reservedImplementations: number;
  readonly verifiedImplementations: number;
  readonly missingBasicTestImplementations: number;
  readonly basicTestPassingImplementations: number;
  readonly releaseReadyImplementations: number;
  /**
   * 두 검증 축이 통과시킨 단정 수의 합. 실패·건너뜀보다 늦게 합류한 집계라,
   * 그 전에 발행된 인덱스에는 없다. 없으면 없는 대로 두고 0으로 지어내지 않는다.
   */
  readonly passedChecks?: number;
  readonly failedChecks: number;
  readonly skippedChecks: number;
...
/**
 * 목록·커버리지·피드백 화면이 읽는 최소 형태다. 2.1 인덱스 항목과 1.x~2.0
 * CatalogAlgorithm이 모두 이 모양을 만족하므로, 화면 헬퍼는 어느 쪽을 받아도 같다.
 */
export interface CatalogListEntry {
  ...
  readonly category: string;
  readonly family: string;
  readonly languages: readonly CatalogLanguageStatus[];
  readonly coverage: {
    readonly vectorCount: number;
    readonly caseCount: number;
    readonly verifiedLanguages: number;
...
export interface CatalogIndexEntry extends CatalogListEntry {
  /**
   * 검색 보조를 아직 받지 못한 동안에는 summary만 채워지고 네 목록은 비어 있다.
   * 없는 문장을 지어내지 않으므로, 그 사이 검색은 이름·분류·요약까지만 걸린다.
   */
  readonly documentation: CatalogDocumentation;
  readonly coverage: {
    readonly vectorCount: number;
  ...
  /** 검색 보조를 받아 네 목록까지 채웠는지. 지연 로드 상태를 화면이 구분한다. */
  readonly searchTextLoaded: boolean;
  readonly detail: CatalogAssetManifest;
}

export interface CatalogIndexDocument {
  readonly schemaVersion: "2.1";
  readonly languages: readonly CatalogLanguageName[];
  readonly coverageFields: readonly string[];
  readonly totals: CatalogIndexTotals;
  readonly search: CatalogAssetManifest;
  readonly algorithms: readonly CatalogIndexEntry[];
}

/** 상세 한 장을 읽어 만든 모델과, 기존 2.0 화면 경로가 쓰는 1종짜리 문서다. */
export interface CatalogDetail {
  readonly algorithm: CatalogAlgorithm;
  /**
   * 이름 → 종별 자산 manifest. 본문은 상세에 실리지 않고 manifest가 가리키는 정적
   * 파일 한 장에 있다. 자산이 없는 종은 빈 객체이고 그것이 정상 상태다 — 화면은
   * 그 영역을 그리지 않으면 된다.
   */
  readonly assets: Readonly<Record<string, CatalogAssetManifest>>;
  /**
   * 인덱스 항목과 상세를 합치면 그 알고리즘의 2.0 항목이 그대로 복원된다(생성기가
   * 역병합 바이트까지 대조해 보장한다). 그래서 상세 화면의 소스·벡터 조회는 이
   * 1종짜리 2.0 문서를 그대로 통과시켜 기존 경로를 한 줄도 바꾸지 않는다.
   */
 ...
 */
const catalogIndexOptionalTotalsFields = ["passedChecks"] as const;
...
function assetManifest(
  ...
  if (
    ...
    throw new TypeError(
      `catalog.json ${field}.path: 정규화된 상대 경로여야 합니다.`,
    );
  }
  if (expectedPath !== undefined && manifestPath !== expectedPath) {
    throw new TypeError(
      `catalog.json ${field}.path: '${expectedPath}'이어야 합니다.`,
    );
  }
  return {
    path: manifestPath,
...
function parseIndexTotals(value: unknown, field: string): CatalogIndexTotals {
  const totals = object(value, field);
  exactShape(
    totals,
    catalogIndexTotalsFields,
  ...
  ]) as unknown as CatalogIndexTotals;
}
 ...
 */
function parseEntryCoverage(
  value: unknown,
  field: string,
): CatalogIndexEntry["coverage"] {
  if (typeof value !== "string" || !coverageStringPattern.test(value)) {
    throw new TypeError(
      `catalog.json ${field}: coverageFields 순서에 대응하는 ${catalogCoverageFields.length}개 십진 정수 목록이어야 합니다.`,
    );
  }
  const parts = value.split(",").map((part) => Number(part));
  return Object.fromEntries(
    catalogCoverageFields.map((key, index) => [key, parts[index]]),
  ) as unknown as CatalogIndexEntry["coverage"];
}
...
function parseIndexEntry(
  ...
  if (
    ...
    throw new TypeError(
      `catalog.json ${field}.implementationStatus: languages 순서에 대응하는 ${languages.length}자리 O/R 문자열이어야 합니다.`,
    );
  }
  return {
    id: algorithmId,
    ...
    detail: assetManifest(
      entry.detail,
      `${field}.detail`,
      `algorithms/${algorithmId}.json`,
    ),
  };
}

/** 목록 인덱스를 읽는다. 상세를 한 장도 받지 않은 상태에서 목록 화면 전체가 나온다. */
export function parseCatalogIndex(value: unknown): CatalogIndexDocument {
  const root = object(value, "$");
  exactKeys(
    root,
    ["schemaVersion", "languages", "coverageFields", "totals", "search", "algorithms"],
    "$",
  );
  ...
  if (
    declaredLanguages.length !== languages.length ||
    declaredLanguages.some((language, index) => language !== languages[index])
  ) {
    throw new TypeError(
      `catalog.json $.languages: 고정 공개 언어 순서 ${languages.join(", ")}와 정확히 같아야 합니다.`,
    );
  }
  const declaredCoverageFields = stringArray(
    root.coverageFields,
    "$.coverageFields",
  );
  if (
    declaredCoverageFields.length !== catalogCoverageFields.length ||
    declaredCoverageFields.some(
      (name, index) => name !== catalogCoverageFields[index],
    )
  ) {
    throw new TypeError(
      `catalog.json $.coverageFields: 고정 커버리지 자리 순서 ${catalogCoverageFields.join(", ")}와 정확히 같아야 합니다.`,
    );
  }
  const algorithmMap = object(root.algorithms, "$.algorithms");
  const algorithmIds = asciiSortedKeys(algorithmMap, "$.algorithms").map((id) =>
    algorithmIdentifier20(id, `$.algorithms.${id}`),
  );
  if (!algorithmIds.length) {
    throw new TypeError(
      "catalog.json $.algorithms: 하나 이상의 알고리즘이 필요합니다.",
  ...
  if (totals.algorithmCount !== algorithms.length) {
    throw new TypeError(
      `catalog.json $.totals.algorithmCount: 인덱스 항목 수 ${algorithms.length}와 일치해야 합니다.`,
    );
  }
  return {
    schemaVersion: "2.1",
 ...
 */
export function applyCatalogSearch(
  ...
  if (orphans.length) {
    throw new TypeError(
      `catalog.json $.algorithms: 인덱스에 없는 검색 본문이 있습니다: ${orphans.join(", ")}.`,
    );
  }
  const algorithms = index.algorithms.map((entry): CatalogIndexEntry => {
    const field = `$.algorithms.${entry.id}`;
    const documentation = object(algorithmMap[entry.id], field);
    exactKeys(
      documentation,
      ["whenToUse", "avoidWhen", "tradeoffs", "pitfalls"],
      field,
    );
    const list = (key: "whenToUse" | "avoidWhen" | "tradeoffs" | "pitfalls") =>
      uniqueStrings(
...
export function parseCatalogDetail(
  ...
  if (algorithmIdentifier20(detail.id, "$.id") !== entry.id) {
    throw new TypeError(
      `catalog.json $.id: 인덱스가 가리킨 '${entry.id}'와 같아야 합니다.`,
    );
  }
  ...
  const assets: Record<string, CatalogAssetManifest> = {};
  ...
  const algorithm = parseAlgorithm20(
    merged,
    entry.id,
    `$.algorithms.${entry.id}`,
    state,
  ...
  for (const targetId of algorithm.alternatives ?? []) {
    if (!knownIds.has(targetId)) {
      throw new TypeError(
        `catalog.json ${entry.id}.alternatives: 존재하지 않는 algorithmId '${targetId}'을 참조합니다.`,
      );
    }
  }
  for (const relationship of relationshipFields) {
    for (const targetId of algorithm.relationships?.[relationship] ?? []) {
      if (!knownIds.has(targetId)) {
        throw new TypeError(
          `catalog.json ${entry.id}.relationships.${relationship}: 존재하지 않는 algorithmId '${targetId}'을 참조합니다.`,
        );
      }
    }
  }
  ...
  for (const key of catalogCoverageFields) {
    ...
    if (declared !== derived) {
      throw new TypeError(
        `catalog.json $.algorithms.${entry.id}.coverage.${key}: 인덱스 선언값 ${declared}과 상세 계산값 ${derived}이 일치하지 않습니다.`,
      );
    }
  }
  algorithm.languages.forEach((language, position) => {
    ...
    if (declared !== language.implementationStatus) {
      throw new TypeError(
        `catalog.json $.algorithms.${entry.id}.implementationStatus: '${language.language}' 상태가 상세와 일치하지 않습니다.`,
      );
    }
  });
  ...
  return {
    ...
    document: {
      ...
      coverage: {
        ...
        languageImplementations: algorithm.languages.reduce(
          (tally, language) => ({
            ...tally,
            [language.implementationStatus]:
              tally[language.implementationStatus] + 1,
          }),
          { R: 0, I: 0, O: 0, E: 0 } as Record<ImplementationStatus, number>,
        ),
        verifiedLanguages: algorithm.coverage.verifiedLanguages,
        missingBasicTests: algorithm.coverage.missingBasicTests ?? 0,
        releaseReadyLanguages: algorithm.coverage.releaseReadyLanguages ?? 0,
...
export function summarizeIndexReadiness(
  ...
  return {
    ...
    basicTestPassingImplementations:
      index.totals.basicTestPassingImplementations,
...
function feedbackRecord(feedback: CatalogFeedback): RecordValue {
...
export function resolveAlternativeAlgorithms<T extends { readonly id: string }>(
  algorithm: { readonly id: string; readonly alternatives?: readonly string[] },
  algorithms: readonly T[],
): readonly T[] {
  if (!algorithm.alternatives?.length) return [];
  ...
  return algorithm.alternatives.map((alternativeId) => {
    ...
    if (!alternative) {
      throw new TypeError(
        `catalog.json ${algorithm.id}.alternatives: 대안 algorithmId '${alternativeId}'을 찾을 수 없습니다.`,
      );
    }
    return alternative;
  });
...
export function sourceForLanguage(
  ...
  if (language.sourceGroup) {
    return language.sourceGroup.files.find(
      (source) => source.path === language.sourceGroup?.primaryFile,
    );
  }
  if (!language.sourceId) return undefined;
 ...
 * 바꾸지 않는다.
 */
export function sourceGroupForLanguage(
  catalog: CatalogDocument,
  language: CatalogLanguage,
): CatalogFileGroup | undefined {
  if (language.sourceGroup) return language.sourceGroup;
...
export function vectorsForAlgorithm(
  catalog: CatalogDocument,
  algorithm: CatalogAlgorithm,
): readonly CatalogVectorManifest[] {
  if (!algorithm.vectorIds) return [];
  const byId = new Map((catalog.vectors ?? []).map((vector) => [vector.id, vector]));
  return algorithm.vectorIds.map((id) => {
    const vector = byId.get(id);
    if (!vector) {
      throw new TypeError(
        `catalog.json ${algorithm.id}.vectorIds: vector '${id}'를 찾을 수 없습니다.`,
      );
    }
    return vector;
  });
...
export function isEmbeddedCatalogSource(
  source: CatalogSource,
): source is CatalogSource & { readonly content: string } {
  return typeof source.content === "string";
}

/** parser를 통과한 1.2 source만 원격 정본 로더로 보낸다. */
export function isRemoteCatalogSource(
  source: CatalogSource,
): source is CatalogSource & {
  readonly sha256: string;
  readonly byteLength: number;
} {
  return (
 ...
 */
const bundledSourceIdCache = new WeakMap<CatalogDocument, ReadonlySet<string>>();

export function bundledSourceIds(
  catalog: CatalogDocument,
): ReadonlySet<string> {
  ...
  for (const algorithm of catalog.algorithms) {
    for (const language of algorithm.languages) {
      ...
      if (owners) owners.add(algorithm.id);
      else referrers.set(language.sourceId, new Set([algorithm.id]));
    }
  }
  ...
  for (const [sourceId, owners] of referrers) {
    ...
    if (owners.size > 1) bundled.add(sourceId);
  }
 ...
 */
export function isBundledSource(
  catalog: CatalogDocument,
  language: { readonly sourceId?: string },
): boolean {
  return (
    language.sourceId !== undefined &&
 ...
 */
const languageDisplayOrder: readonly CatalogLanguageName[] = [
  "java",
  "javascript",
  "csharp",
  "typescript",
...
function languageDisplayRank(language: CatalogLanguageName): number {
...
export function sortedLanguages<
  Language extends { readonly language: CatalogLanguageName },
>(languages: readonly Language[]): readonly Language[] {
  return [...languages].sort((left, right) => {
    const rankDifference =
      languageDisplayRank(left.language) - languageDisplayRank(right.language);
 ...
 */
export function filterAlgorithms<Entry extends CatalogListEntry>(
  algorithms: readonly Entry[],
  query: string,
  category = "",
): readonly Entry[] {
...
export function summarizeFeedback(
  algorithms: readonly { readonly feedback: CatalogFeedback }[],
): FeedbackSummary {
...
export function summarizeCatalogReadiness(
  ...
  for (const algorithm of catalog.algorithms) {
    for (const language of algorithm.languages) {
      ...
      const basicTestPassing =
        language.basicTest != null &&
        basicVerification?.status === "passing" &&
        basicVerification.failed === 0 &&
        basicVerification.skipped === 0;
      if (basicTestPassing) basicTestPassingImplementations += 1;

      const vectorPassing =
        vectorVerification.status === "passing" &&
...
export function algorithmHash(id: string): string {
  return `#algorithm/${encodeURIComponent(id)}`;
}

export function algorithmIdFromHash(hash: string): string | undefined {
