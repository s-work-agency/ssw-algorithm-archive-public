...
import type { NameLanguage } from "./korean-display-names.js";

/**
 * UI 언어. 20차의 이름 표기 언어가 넓어진 것이라 타입을 새로 만들지 않고 그대로
 * 쓴다. 토글 하나가 이름 표기와 UI 크롬을 함께 지배한다.
 */
export type UiLanguage = NameLanguage;

/** 의미 기반 키 → 화면 문자열 하나. 자리표시자가 있으면 {name} 꼴로 담는다. */
export type UiStringTable = Readonly<Record<string, string>>;

/** 키 집합은 englishUiStrings와 전수로 일치해야 한다(테스트가 확인한다). */
export const koreanUiStrings: UiStringTable = {
  // 문서·앱 셸
  "document.description":
    "설명·계약·테스트·언어별 원본 코드와 커버리지 현황을 제공하는 SSW Algorithm Archive",
  ...
  */
  ...
  "status.startupFailed":
    "화면을 초기화하지 못했습니다. 원인 유형: {errorType}. 배포 HTML을 확인하세요.",
  "status.dataFailed":
    "{message} 원인 유형: {errorType}. 배포 루트의 데이터 계약을 확인하세요.",
  ...
  "list.searchAidFailed":
    "선택 가이드 본문을 불러오지 못해 검색이 이름·요약·분류까지만 걸립니다.",
  ...
  "detail.loadFailed":
    "알고리즘 상세를 불러오지 못했습니다. 파일이 없거나 인덱스가 발표한 해시·길이와 다릅니다.",
  ...
  "compare.columnBasicTestVerification":
    "기본 테스트 검증 (통과/실패/건너뜀)",
  ...
  "compare.emptySourceContent":
    "게시된 원본 소스 파일이 비어 있습니다.",
  "compare.sourceLoading": "검증된 원본 소스를 불러오는 중입니다…",
  "compare.sourceLoadFailed":
    "원본 소스를 불러오거나 무결성을 확인하지 못했습니다. 목록과 설명은 계속 볼 수 있습니다.",
  "compare.retrySource": "소스 다시 불러오기",
  "compare.noPublishedSource":
    "소스가 게시되지 않은 구현입니다. 엔트리포인트와 검증 요약만 표시합니다.",
  ...
  "guidance.missingDocumentation":
    "이 catalog {schema}에는 확장 설명이 없습니다. 요약과 계약 정보는 그대로 표시합니다.",
  ...
  "tests.vectorLoadFailed":
    "테스트 벡터를 불러오거나 무결성을 확인하지 못했습니다.",
  ...
  "bundled.cardNote":
    "여러 알고리즘이 함께 쓰는 번들 소스라 이 알고리즘만의 자립 소스가 아직 없습니다. 검증은 통과했고, 자립 파일로 분리되면 이 자리에 코드가 나옵니다.",
  ...
  "run.running": "실행 중입니다…",
  "run.setupFailed":
    "소스 또는 벡터를 읽지 못해 실행기를 준비하지 못했습니다. 코드 표시와 같은 무결성 검사를 통과해야 실행할 수 있습니다.",
...
export const englishUiStrings: UiStringTable = {
  // Document and app shell
  "document.description":
    "SSW Algorithm Archive — guidance, contracts, tests, per-language sources and coverage status",
  ...
  "status.startupFailed":
    "Could not initialize the page. Error type: {errorType}. Check the deployed HTML.",
  "status.dataFailed":
    "{message} Error type: {errorType}. Check the data contract at the deployment root.",
  ...
  "list.searchAidFailed":
    "The selection guidance text could not be loaded, so search only covers names, summaries and categories.",
  ...
  "detail.empty":
    "Pick an algorithm from the list to see its guidance, contract, tests and implementations.",
  "detail.loading": "Loading the algorithm detail…",
  "detail.loadFailed":
    "Could not load the algorithm detail. The file is missing or does not match the hash and length the index published.",
  ...
  "compare.columnVectorVerification":
    "Official vectors (passed/failed/skipped)",
  "compare.columnBasicTestVerification":
    "Basic tests (passed/failed/skipped)",
  ...
  "compare.emptySourceContent":
    "The published source file is empty.",
  "compare.sourceLoading": "Loading and verifying the canonical source…",
  "compare.sourceLoadFailed":
    "The source could not be loaded or verified. The list and documentation remain available.",
  "compare.retrySource": "Retry source",
  "compare.noPublishedSource":
    "This implementation publishes no source. Only the entry point and verification summary are shown.",
  "compare.missingCatalog":
    "There is no catalog to draw implementation details from.",
  ...
  "guidance.missingDocumentation":
    "This catalog {schema} publishes no extended documentation. The summary and contract are shown as they are.",
  ...
  "mutation.input-must-not-mutate":
    "Immutable input — the input is never modified",
  ...
  "tests.vectorLoadFailed":
    "The test vector could not be loaded or verified.",
  ...
  "bundled.cardNote":
    "This source is a bundle shared by several algorithms, so a standalone source for this algorithm alone does not exist yet. Verification passes, and the code will appear here once the file is split out.",
  ...
  "run.running": "Running…",
  "run.setupFailed":
    "Could not read the source or the vector, so the runner is unavailable. Running requires the same integrity check the code display passes.",
  ...
  "run.timeoutDetail":
    "It did not finish within {timeout}s, so the worker was terminated.",
  "run.missingExport": "The source does not export {name}.",
  "run.crashedDetail": "The run stopped: {message}",
};

function tableForLanguage(language: UiLanguage): UiStringTable {
 ...
 */
function formatUiString(
  template: string,
  values: Readonly<Record<string, string | number>>,
): string {
  return template.replace(/\{(\w+)\}/gu, (match: string, name: string) =>
 ...
 */
export function optionalUiString(
  key: string,
  language: UiLanguage,
  values?: Readonly<Record<string, string | number>>,
): string | undefined {
  ...
  const template =
    lookupUiString(tableForLanguage(language), key) ??
...
/** 상태줄 지표 한 줄. 라벨·값 두 열은 상세 화면 .metrics 의 dl 관례를 따른다. */
export interface StatusMetricRow {
  readonly label: string;
  readonly value: string;
}

/** 상태줄이 싣는 수치. 인덱스 totals에서 뽑아 온 것만 담는다. */
export interface StatusMetricTotals {
 ...
 */
export function statusMetricRows(
  totals: StatusMetricTotals,
  language: UiLanguage,
): readonly StatusMetricRow[] {
