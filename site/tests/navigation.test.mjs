...
test("소개 본문 안 절 앵커는 화면을 소개에 묶어 둔다", () => {
  ...
  */
  for (const hash of [
    "#about/1-가볍게-시작했습니다",
    "#about/6-그래서-이-사이트에-이것을-담았습니다",
    // 주소창을 지나며 퍼센트 인코딩된 형태도 같은 화면이어야 한다.
    "#about/5-%EC%9A%B0%EC%84%A0%EC%9D%80",
  ]) {
...
test("소개 → 목록 → 상세 → 소개 왕복이 화면 판정만으로 닫힌다", () => {
  // 사이드바 소개 · 사이드바 목록 · 카드 클릭 · 다시 사이드바 소개 순서다.
  const trip = [
    ["", "about"],
    ["#list", "list"],
...
test("소개를 보는 중에 카탈로그 네비를 누르면 화면을 다시 그린다", () => {
  // 첫 화면이 소개라 이 경로가 가장 흔한 목록 진입이다. "tab" 이면 소개 본문이
...
test("선택 id 만으로는 화면 전환을 알 수 없다", () => {
  /*
    ...
    하는 이유이기도 하다.
  */
  for (const hash of ["#list", "#coverage", aboutHash]) {
    assert.equal(
