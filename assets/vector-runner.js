...
export function javaScriptAdapter(algorithmId) {
    return Object.hasOwn(adapters, algorithmId) ? adapters[algorithmId] : undefined;
}
export function parseVectorCase(value) {
    ...
    const errorRecord = expectedRecord.error;
    const normalizedExpected = errorRecord && typeof errorRecord.code === "string"
...
export function runJavaScriptCase(options) {
    return new Promise((resolve) => {
        ...
        const timer = window.setTimeout(() => finish({ status: "timeout" }), options.timeoutMilliseconds ?? defaultRunTimeoutMilliseconds);
 ...
 */
export function judgeCase(expected, outcome) {
    ...
    if ("error" in expected) {
        if (outcome.status !== "error")
            return { kind: "failed", reason: "shape" };
