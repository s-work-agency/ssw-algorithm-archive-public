...
export const koreanUiStrings = {
    ...
    */
    ...
    "run.title": "브라우저에서 실행",
    "run.intro": "위 소스를 그대로 브라우저의 별도 스레드(Web Worker)에서 격리 실행합니다. 입력은 공식 벡터의 케이스이고, 결과는 그 케이스가 선언한 기대값과 대조합니다. {timeout}초를 넘기면 워커를 강제 종료합니다.",
...
export const englishUiStrings = {
    ...
    "run.intro": "Runs the source above as-is on a separate browser thread (Web Worker), isolated from the page. The input comes from an official vector case, and the result is checked against what that case declares. The worker is force-terminated after {timeout}s.",
