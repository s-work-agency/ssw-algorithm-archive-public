 ...
 */
export function usageScenarioManifest(assets) {
...
function paragraphs(value, field) {
    if (!Array.isArray(value) || !value.length) {
        throw new TypeError(`usage-scenario.json ${field}: 비어 있지 않은 배열이어야 합니다.`);
    }
    return value.map((paragraph, index) => {
        if (typeof paragraph !== "string" || !paragraph.length) {
            throw new TypeError(`usage-scenario.json ${field}[${index}]: 비어 있지 않은 문자열이어야 합니다.`);
        }
        return paragraph;
    });
}
 ...
 */
export function parseUsageScenario(value, algorithmId) {
    if (!isRecord(value)) {
    ...
    if (keys.length !== expectedKeys.length ||
        ...
        throw new TypeError(`usage-scenario.json $: ${expectedKeys.join(", ")} 여섯 필드만 있어야 합니다.`);
    }
    if (value.assetVersion !== usageScenarioAssetVersion) {
        throw new TypeError(`usage-scenario.json $.assetVersion: '${usageScenarioAssetVersion}'이어야 합니다.`);
    }
    // 자산은 상세를 거쳐 왔지만 소유 알고리즘을 자기 안에도 적는다. 어긋나면 다른
    // 종의 시나리오를 이 화면에 붙이는 것이므로 그리지 않는다.
    if (value.algorithmId !== algorithmId) {
        throw new TypeError(`usage-scenario.json $.algorithmId: 상세가 가리킨 '${algorithmId}'와 같아야 합니다.`);
    }
    return {
