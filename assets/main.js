import { algorithmHash, algorithmIdFromHash, applyCatalogSearch, contractPresentation, filterAlgorithms, isEmbeddedCatalogSource, isBundledSource, isRemoteCatalogSource, parseCatalogDetail, parseCatalogIndex, resolveAlternativeAlgorithms, sortedLanguages, sourceGroupForLanguage, summarizeIndexReadiness, vectorsForAlgorithm, } from "./catalog.js";
import { algorithmDisplayName, categoryDisplayName, familyDisplayName, } from "./korean-display-names.js";
import { aboutHash, isAboutHash, isDocumentScreen, isThoughtsHash, catalogTabActivation, screenForHash, scrollBehaviorForPreference, selectedIdForNavigation, thoughtsHash, } from "./navigation.js";
import { parseUsageScenario, usageScenarioManifest, } from "./usage-scenario.js";
import { optionalUiString, statusMetricRows, uiString, } from "./ui-strings.js";
import { defaultRunTimeoutMilliseconds, javaScriptAdapter, judgeCase, parseVectorCase, runJavaScriptCase, } from "./vector-runner.js";
import { AssetViewGeneration, VerifiedStaticJsonLoader, VerifiedStaticTextLoader, parseVerifiedVector, settleAssetRequest, vendoredAssetManifest, vendoredSourceRoot, vendoredVectorRoot, } from "./remote-assets.js";
/**
 ...
 * 보내도 기호 없는 칩이 되지 않는다.
 */
const implementationStatusMarks = {
    O: "✓",
    I: "◐",
    R: "○",
    E: "×",
 ...
 */
const shownImplementationStatus = "O";
function operationalLanguages(languages) {
    return languages.filter((language) => language.implementationStatus === shownImplementationStatus);
}
/**
...
function isListViewMode(value) {
    return value !== null && listViewModes.includes(value);
}
...
function readStoredListView() {
    try {
        ...
        console.error(`목록 보기 설정을 읽지 못했습니다. 원인 유형: ${errorType(error)}`, error);
        return "card";
    }
}
function storeListView(mode) {
    try {
        window.localStorage.setItem(listViewStorageKey, mode);
    }
    catch (error) {
        console.error(`목록 보기 설정을 저장하지 못했습니다. 원인 유형: ${errorType(error)}`, error);
    }
}
/**
...
function readStoredTheme() {
    try {
        ...
        console.error(`테마 설정을 읽지 못했습니다. 원인 유형: ${errorType(error)}`, error);
        return "light";
    }
}
function storeTheme(theme) {
    try {
        ...
        console.error(`테마 설정을 저장하지 못했습니다. 원인 유형: ${errorType(error)}`, error);
    }
}
/**
...
function readStoredNameLanguage() {
    try {
        ...
        console.error(`이름 표기 설정을 읽지 못했습니다. 원인 유형: ${errorType(error)}`, error);
        return "ko";
    }
}
function storeNameLanguage(language) {
    try {
        window.localStorage.setItem(languageStorageKey, language);
    }
    catch (error) {
        console.error(`이름 표기 설정을 저장하지 못했습니다. 원인 유형: ${errorType(error)}`, error);
    }
}
/**
 ...
 */
function catalogTabFromHash(hash) {
    if (!hash || hash === "#")
        return "list";
    return catalogTabDefinitions.find((definition) => definition.hash === hash)
        ?.id;
}
function catalogTabHash(tab) {
    return (catalogTabDefinitions.find((definition) => definition.id === tab)?.hash ??
        "#list");
}
...
function reportStartupFailure(error) {
    ...
    if (status) {
        ...
        status.textContent = uiString("status.startupFailed", readStoredNameLanguage(), { errorType: errorType(error) });
...
function main() {
    function required(selector) {
        ...
        if (!item)
            throw new Error(`필수 DOM 요소가 없습니다: ${selector}`);
        return item;
    }
    const search = required("#algorithm-search");
    const category = required("#category-filter");
    const list = required("#algorithm-list");
    const listHeading = required("#catalog-title");
    const status = required("#catalog-status");
    const resultCount = required("#result-count");
    const detail = required("#detail-content");
    const detailPanel = required("#detail");
    const matrix = required("#coverage-matrix");
    const catalogScreen = required("#catalog-screen");
    /*
      ...
      선택자는 흔들리지 않는다.
    */
    const aboutPanel = required("#about-panel");
    const aboutDoc = required("#about-doc");
    const aboutNavLink = required("#nav-about");
    const aboutCta = required("#about-cta");
    const thoughtsPanel = required("#thoughts-panel");
    const thoughtsDoc = required("#thoughts-doc");
    const thoughtsNavLink = required("#nav-thoughts");
    const skipLink = required(".skip-link");
    const mainContent = required("#main-content");
    const topbar = required(".topbar");
    const brandHome = required("#brand-home");
    const sidebar = required("#sidebar");
    const sidebarScrim = required("#sidebar-scrim");
    const sidebarToggle = required("#sidebar-toggle");
    const screenTitle = required("#screen-title");
    const themeToggle = required("#theme-toggle");
    const themeToggleIcon = required("#theme-toggle .theme-toggle__icon");
    ...
    const nameLanguageButtons = new Map([
        ["ko", required("#name-language-ko")],
        ["en", required("#name-language-en")],
    ]);
    const viewCardButton = required("#view-card");
    const viewRowsButton = required("#view-rows");
    const copyStatus = required("#copy-status");
    const catalogTabControls = new Map(catalogTabDefinitions.map((definition) => [
        definition.id,
        {
            tab: required(`#${definition.tabElementId}`),
            panel: required(`#${definition.panelElementId}`),
        },
    ]));
     ...
     */
    let detailCatalog = null;
    let detailAlgorithm = null;
    /**
     ...
     */
    let detailAssets = null;
    let detailScenario = null;
    let searchAidStatus = "idle";
    let selectedId = "";
    ...
    let screen = "about";
    let listScrollY = 0;
    let listScrollTab = "list";
    let activeSpecTab = "overview";
    let activeCatalogTab = catalogTabFromHash(window.location.hash) ?? "list";
    let listViewMode = readStoredListView();
    let activeTheme = readStoredTheme();
    let activeNameLanguage = readStoredNameLanguage();
    /** 비보안 컨텍스트에는 clipboard API가 없으므로 복사 UI 자체를 만들지 않는다. */
    const clipboard = navigator.clipboard;
    const writeToClipboard = clipboard && typeof clipboard.writeText === "function"
     ...
     */
    const t = (key, values) => uiString(key, activeNameLanguage, values);
    /**
     ...
     */
    const displayName = (algorithm) => algorithmDisplayName(algorithm.id, algorithm.name, activeNameLanguage);
    const displayCategory = (value) => categoryDisplayName(value, activeNameLanguage);
    const displayFamily = (value) => familyDisplayName(value, activeNameLanguage);
    /**
     * 구현 상태(R·I·O·E)는 catalog가 코드로 발행하고 화면에는 말로 찍힌다.
     * catalog.ts의 statusLabel은 한국어 한 벌뿐이라 여기서는 로케일 표를 거친다.
     */
    const implementationStatusText = (status) => t(`implementationStatus.${status}`);
    const preferredScrollBehavior = () => scrollBehaviorForPreference(window.matchMedia("(prefers-reduced-motion: reduce)").matches);
    function focusSelectedDetail(scroll) {
        if (!selectedId)
            return;
        const heading = detail.querySelector("[data-detail-heading]");
    ...
    function applyNameLanguage() {
        for (const [language, button] of nameLanguageButtons) {
            button.setAttribute("aria-pressed", String(language === activeNameLanguage));
        }
    }
    ...
    function applyStaticUiStrings() {
        ...
        */
        if (screen !== "detail")
            screenTitle.textContent = activeScreenTitle();
    }
    /**
     ...
     */
    function selectNameLanguage(language) {
        if (activeNameLanguage === language)
            return;
    ...
    function activeScreenTitle() {
        if (screen === "about")
            return t("screen.about");
        // 생각만 탑바가 글의 제목을 든다. 본문에서는 그 제목을 뺐다(build-pages.mjs).
        if (screen === "thoughts")
            return t("screen.thoughts");
        const definition = catalogTabDefinitions.find((item) => item.id === activeCatalogTab);
    ...
    function applyCatalogTabs() {
        ...
        for (const definition of catalogTabDefinitions) {
            ...
            if (active && !onDocument) {
                ...
                control.tab.removeAttribute("aria-current");
    ...
    function ensureContentVisible() {
        if (catalogScreen.getBoundingClientRect().top >= 0)
            return;
     ...
     */
    function selectCatalogTab(tab, options = {}) {
    ...
    function focusListEntry(scroll) {
        if (scroll) {
        ...
        button.addEventListener("click", () => {
            void (async () => {
                try {
                    ...
                    console.error(`코드를 복사하지 못했습니다. 원인 유형: ${errorType(error)}`, error);
    ...
    function retryButton(action, labelKey) {
     ...
     */
    function appendRemoteSource(card, source, generation, labels = {
        ...
        const attempt = () => {
            ...
            const request = staticTextLoader.load(vendoredAssetManifest(vendoredSourceRoot, {
                path: source.path,
            ...
            void settleAssetRequest({
                ...
                onSuccess: (loaded) => {
                    ...
                    host.replaceChildren(loaded.text ? sourceCodeBlock(loaded.text) : show(t("compare.emptySourceContent")));
                },
                onFailure: (error) => {
     ...
     */
    function reportBrowserFailure(error, messageKey) {
        console.error(`${uiString(messageKey, "ko")} 원인 유형: ${errorType(error)}`, error);
        catalogIndex = null;
        detailCatalog = null;
        ...
        try {
            ...
            console.error(`오류 화면 렌더링에 실패했습니다. 원인 유형: ${errorType(renderError)}`, renderError);
        }
        setStatus(() => t("status.dataFailed", {
            message: t(messageKey),
            errorType: errorType(error),
        }), "error");
    }
    function selectAlgorithm(id, options = {}) {
    ...
    function algorithmLink(algorithm, label = displayName(algorithm)) {
        const link = element("a", "link-button");
        link.href = algorithmHash(algorithm.id);
        link.textContent = label;
        if (algorithm.id === selectedId)
    ...
    function algorithmEntry(algorithm) {
    ...
    function applyListView() {
        viewCardButton.setAttribute("aria-pressed", String(listViewMode === "card"));
        viewRowsButton.setAttribute("aria-pressed", String(listViewMode === "rows"));
        list.className =
     ...
     * 알리고 다시 받을 길을 함께 준다.
     */
    function searchAidNotice() {
        if (searchAidStatus !== "failed" || !search.value.trim())
            return undefined;
        const notice = element("div", "search-aid-notice");
        notice.append(show(t("list.searchAidFailed")), retryButton(() => {
    ...
    function renderList() {
        ...
        if (!algorithms.length) {
            list.append(show(catalogIndex ? t("list.emptyFiltered") : t("list.emptyCatalog")));
            return;
        }
    ...
    function appendBulletSection(parent, title, values) {
    ...
    function missingDocumentationNote() {
        return show(t("guidance.missingDocumentation", {
            schema: catalogIndex?.schemaVersion ?? "",
        }));
    }
    function buildGuidanceGrid(selected) {
    ...
    function fieldGroupRows(body, label, fields, format) {
    ...
    function contractTableHead(labels) {
    ...
    function contractInterfaceTable(presentation) {
        const wrap = element("div", "table-wrap contract-table-wrap");
        const table = element("table", "data-table contract-table");
        const body = element("tbody");
        const describe = (value) => typeof value === "string" ? value : compactValue(value);
        fieldGroupRows(body, t("contract.input"), presentation.input, describe);
        fieldGroupRows(body, t("contract.output"), presentation.output, describe);
        table.append(contractTableHead([
    ...
    function contractErrorsCard(errors) {
        ...
        for (const error of errors) {
            ...
            if (code && extras.length) {
                ...
                detail.textContent = extras
                    .map(([key, value]) => `${key}: ${typeof value === "string" ? value : compactValue(value)}`)
                    .join(" · ");
                item.append(detail);
            }
            list.append(item);
     ...
     */
    function contractMutationCard(mutation) {
        const card = contractCard(t("contract.mutationTitle"));
        if (mutation === undefined) {
            card.append(emptyNote(t("contract.emptyMutation")));
            return card;
        }
        const label = optionalUiString(`mutation.${mutation}`, activeNameLanguage);
        if (label) {
    ...
    function contractPreconditionsCard(selected) {
     ...
     * 목록에 원본 JSON 그대로 남는다.
     */
    function contractExample(selected) {
        ...
        const heading = element("h5");
        const caseId = typeof record.caseId === "string" ? record.caseId : undefined;
        ...
        const body = element("tbody");
        if (input)
            fieldGroupRows(body, t("contract.input"), input, compactValue);
        if (outcome)
            fieldGroupRows(body, outcomeLabel, outcome, compactValue);
        table.append(contractTableHead([
            t("contract.columnGroup"),
            t("contract.columnField"),
    ...
    function renderContract(selected) {
        ...
        grid.append(contractErrorsCard(presentation.errors), contractMutationCard(presentation.mutation), contractPreconditionsCard(selected));
    ...
    function appendTestCases(parent, cases) {
        cases.forEach((testCase, index) => {
            const card = element("article", "test-case");
            const caseHeading = element("h5");
            const caseRecord = typeof testCase === "object" &&
                testCase !== null &&
    ...
    function remoteVectorDisclosure(manifest, generation) {
        ...
        const attempt = () => {
            ...
            void settleAssetRequest({
                ...
                onSuccess: (loaded) => {
                    ...
                    if (!loaded.vector.cases.length)
                        host.append(show(t("tests.emptyCases")));
                },
                onFailure: (error) => {
                    console.error(`테스트 벡터를 읽지 못했습니다: ${manifest.id}`, error);
                    host.removeAttribute("aria-busy");
                    host.replaceChildren(show(t("tests.vectorLoadFailed")), retryButton(attempt, "tests.retryVector"));
                },
            });
        };
    ...
    function renderTestVectors(selected) {
        const section = element("section", "detail-section");
        const heading = element("h4");
        ...
        if (selected.vectorIds) {
            ...
            for (const manifest of manifests) {
                section.append(remoteVectorDisclosure(manifest, currentAssetViewGeneration));
            }
            return section;
        }
    ...
    function buildMetrics(selected) {
    ...
    function buildAlternatives(selected) {
        // 대안은 다른 종을 가리키므로 상세 한 장이 아니라 인덱스에서 이름을 찾는다.
        const alternatives = resolveAlternativeAlgorithms(selected, catalogIndex?.algorithms ?? []);
     ...
     */
    function buildScenarioMount(selected, generation) {
        ...
        void settleAssetRequest({
            ...
            onFailure: (error) => {
                console.debug(`사용 시나리오 자산을 읽지 못해 그 영역을 비웁니다: ${manifest.path}`, error);
    ...
    function legacyVectorVerification(language) {
        return (language.vectorVerification ?? {
    ...
    function appendCatalogSource(parent, source, generation) {
    ...
    function sourceFileGroup(group, generation) {
     ...
     * 순서는 건드리지 않는다.
     */
    function sourceCardOrder(languages) {
        const rank = (language) => {
    ...
    function implementationSourceCard(selected, language, generation) {
        // 상세를 받아 만든 1종짜리 2.0 문서다. 소스 조회 경로는 2.0 때와 같다.
        const catalog = detailCatalog;
        if (!catalog)
            return show(t("compare.missingCatalog"));
     ...
     */
    function buildVectorRunner(selected, group, generation) {
        ...
        let sourceText = "";
        let cases = [];
        const selectedCase = () => cases[Number(caseSelect.value)];
        const fillInput = () => {
        ...
        const paintVerdict = (current, outcome, edited) => {
            ...
            const badgeStatus = verdict.kind === "passed"
                ? "approved"
                : verdict.kind === "failed"
                    ? "rejected"
                    : "pending";
            ...
            const expectedPane = pane(t("run.expected"), jsonBlock("error" in current.expected
        ...
        const ready = Promise.all([
            staticTextLoader.load(vendoredAssetManifest(vendoredSourceRoot, sourceManifest)),
            staticTextLoader
            ...
            if (!cases.length)
                throw new Error("실행할 수 있는 벡터 케이스가 없습니다.");
        });
        void settleAssetRequest({
            request: ready,
            generation,
            ...
            onSuccess: () => {
                caseSelect.replaceChildren(...cases.map((item, index) => {
                    const option = new Option(item.description
                        ? `${item.caseId} — ${item.description}`
                        : item.caseId, String(index));
                    return option;
                }));
     ...
     */
    function buildComparison(selected, headingLevel = "h3") {
        const catalog = detailCatalog;
        if (!catalog)
            return [];
        ...
        // 카드가 있는 언어가 어긋나지 않는다.
        for (const language of operationalLanguages(sortedLanguages(selected.languages))) {
            const group = sourceGroupForLanguage(catalog, language);
            const bundled = isBundledSource(catalog, language);
            const vectorSummary = legacyVectorVerification(language);
            const basicVerification = catalog.schemaVersion === "2.0"
        ...
        for (const language of operationalLanguages(sourceCardOrder(selected.languages))) {
            sourceGrid.append(implementationSourceCard(selected, language, currentAssetViewGeneration));
        }
    ...
    function implementationContent(selected) {
        if (!detailCatalog)
            return [show(t("compare.missingCatalog"))];
        return buildComparison(selected, "h4");
    }
    function specTabContent(selected, tab) {
        if (tab === "overview")
            return overviewContent(selected);
        if (tab === "spec")
            return specContent(selected);
     ...
     */
    function renderSpecTabs(selected) {
        const wrapper = element("div", "spec-tabs");
        const tablist = element("div", "spec-tablist");
        tablist.setAttribute("role", "tablist");
        tablist.setAttribute("aria-label", t("specTab.listLabel", { name: displayName(selected) }));
        const buttons = [];
        const panels = [];
        const renderedTabs = new Set();
        const ensurePanelContent = (definition, panel) => {
            if (renderedTabs.has(definition.id))
                return;
        ...
        const applyActiveTab = () => {
            specTabDefinitions.forEach((definition, index) => {
                ...
                if (active)
                    ensurePanelContent(definition, panel);
        ...
        tablist.addEventListener("keydown", (event) => {
            const current = specTabDefinitions.findIndex((definition) => definition.id === activeSpecTab);
     ...
     * 화면은 그대로 뜬다.
     */
    function renderDetailBody(entry, body, generation) {
        ...
        void settleAssetRequest({
            ...
            onFailure: (error) => {
                console.error(`알고리즘 상세를 읽지 못했습니다: ${entry.detail.path}`, error);
                body.removeAttribute("aria-busy");
                body.replaceChildren(show(t("detail.loadFailed")), retryButton(() => renderDetailBody(entry, body, generation), "detail.retry"));
            },
        });
    }
     ...
     */
    function implementationStatusChip(status, label, spokenStatus, options = {}) {
        const chip = element("span", "coverage-status");
        chip.dataset.status = status;
        // 번들 여부는 구현 상태와 다른 축이라 상태를 덮지 않고 따로 표시한다.
        if (options.bundled)
            chip.dataset.source = "bundled";
     ...
     */
    function bundledLanguage(language) {
        const catalog = detailCatalog;
     ...
     * 텍스트가 그대로 나른다.
     */
    function coverageStatusCell(algorithm) {
        const cell = element("td");
        const shown = operationalLanguages(sortedLanguages(algorithm.languages));
        if (!shown.length) {
        ...
        for (const language of shown) {
            ...
            item.append(implementationStatusChip(language.implementationStatus, language.language, bundled ? `${spoken} · ${t("bundled.label")}` : spoken, { bundled }));
    ...
    function availableAlgorithmIds() {
    ...
    function render() {
        const selected = catalogIndex?.algorithms.find((algorithm) => algorithm.id === selectedId) ??
    ...
    function populateCategories() {
        const categories = [
            ...new Set(catalogIndex?.algorithms.map((algorithm) => algorithm.category) ?? []),
        ].sort();
        const selectedCategory = category.value;
        category.replaceChildren(new Option(t("list.allCategories"), ""), ...categories.map((value) => new Option(displayCategory(value), value)));
        // 표기를 바꾸느라 다시 채워도 고르고 있던 분류는 그대로 둔다.
        if (selectedCategory && categories.includes(selectedCategory)) {
            category.value = selectedCategory;
        }
    }
    function applyNavigation(scroll) {
        if (!catalogIndex)
            return;
        const nextSelectedId = selectedIdForNavigation(window.location.hash, availableAlgorithmIds(), initialSelectedId);
        const nextScreen = nextScreenForHash();
        const nextCatalogTab = nextScreen === "list"
            ? (catalogTabFromHash(window.location.hash) ?? activeCatalogTab)
            : activeCatalogTab;
        const selectionChanged = nextSelectedId !== undefined && nextSelectedId !== selectedId;
        const screenChanged = nextScreen !== screen;
        const catalogTabChanged = nextCatalogTab !== activeCatalogTab;
        if (!selectionChanged && !screenChanged && !catalogTabChanged)
            return;
        if (selectionChanged) {
        ...
        if (selectionChanged || screenChanged) {
            render();
            if (screen === "detail")
                focusSelectedDetail(scroll);
            else if (isDocumentScreen(screen))
                focusDocument(scroll);
            else
                focusListEntry(scroll);
            return;
        }
     ...
     */
    function loadSearchAid() {
        const index = catalogIndex;
        if (!index || searchAidStatus !== "idle")
            return;
        searchAidStatus = "loading";
        void staticJsonLoader.load(index.search).then((loaded) => {
            ...
            console.error(`검색 보조를 읽지 못했습니다: ${index.search.path}`, error);
            if (catalogIndex !== index)
                return;
    ...
    function scheduleSearchAid() {
        const idle = window.requestIdleCallback;
        if (typeof idle === "function")
            idle(() => loadSearchAid());
        else
            window.setTimeout(() => loadSearchAid(), 1000);
    }
    ...
    async function load() {
        ...
        try {
            ...
            // 이 문서가 발표한 해시로 검증한다.
            const response = await fetch(new URL("./catalog-index.json", window.location.href));
            if (!response.ok)
            ...
            initialSelectedId =
                deepLinkedId &&
                    catalogIndex.algorithms.some((algorithm) => algorithm.id === deepLinkedId)
                    ? deepLinkedId
                    : "";
    ...
    */
    ...
    for (const backLink of document.querySelectorAll("[data-back-to-list]")) {
    ...
    void load().catch((error) => reportBrowserFailure(error, "error.catalogRead"));
}
try {
    main();
}
