 ...
 */
export async function settleAssetRequest(options) {
    try {
        const value = await options.request;
        if (!options.guard.isCurrent(options.generation) ||
            !options.isConnected()) {
            return "stale";
        }
...
function byteDigest(bytes, cryptoProvider) {
    return cryptoProvider.subtle.digest("SHA-256", bytes).then((digest) => Array.from(new Uint8Array(digest), (byte) => byte.toString(16).padStart(2, "0")).join(""));
}
...
export function vendoredAssetManifest(root, manifest) {
    return {
 ...
 */
export function resolveStaticAssetUrl(assetPath, baseUrl) {
...
class VerifiedStaticLoader {
    ...
    load(manifest) {
        ...
        if (cached)
            return cached;
        let request;
        request = this.#load(manifest).catch((error) => {
    ...
    async #load(manifest) {
        ...
        if (!Number.isSafeInteger(manifest.byteLength) ||
...
export class VerifiedStaticJsonLoader extends VerifiedStaticLoader {
    accept() {
        return "application/json";
    }
    finish(text, sha256, byteLength) {
 ...
 */
export class VerifiedStaticTextLoader extends VerifiedStaticLoader {
    accept() {
        return "text/plain";
    }
    finish(text, sha256, byteLength) {
 ...
 */
export function parseVerifiedVector(loaded, manifest) {
    let parsed;
    try {
